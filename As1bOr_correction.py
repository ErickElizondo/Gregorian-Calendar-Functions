...
Instituto Tecnológico de Costa Rica
    ...
    Asignación 1b:
    Programas alrededor del dominio del calendario gregoriano, extensión.
'''
...
nombres_meses = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio",
                 ...
                 "Diciembre"]
# Arreglo de cantidad de días según el mes (Febrero modificable)
cantidades_meses = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
# |--------------------------------------------------------------------------|
...
# Definición: Todas las fechas serán creadas como tuplas de tres
# números enteros positivos (ternas),
# en este orden: (año, mes, día). El resultado debe ser un
...
def fecha_es_tupla(fecha):
    if len(fecha) == 3:  # Se revisa si es una terna
        ...
        if type(fecha[0]) == int and type(fecha[1]) == int and type(fecha[2]) == int:
            # Revisión año mayor que institución calendario Gregoriano
...
def bisiesto(year):
    # Se revisa si es el cambio de un siglo como 1700 o 1800
    if (year % 100) == 0:
        # Si es un siglo y es divisible de 400 es bisiesto
        if (year % 400) == 0:
            return True
    else:
        # Si no es un cambio de siglo y es divible entre 4 es bisiesto
        if (year % 4) == 0:
            return True
    return False
...
def validar_dia_mes(ano, mes, dia):
    if (cantidades_meses[mes-1] == 31 and dia >= 1):
        return (dia <= 31)
    elif (cantidades_meses[mes-1] == 30 and dia >= 1):
        return (dia <= 30)
...
# (tupla de 3 números enteros positivos, que corresponde a una
# fecha en el calendario gregoriano,
# conforme a nuestra convención),el resultado debe ser
# un valor booleano, True or False

def dia_siguiente(fecha):
    # Se valida si la fecha ingresada es válida,
    # de lo contrario se retorna False
    if(fecha_es_valida(fecha)):
        # Comprobación en los respectivos arreglos de meses
        if (cantidades_meses[fecha[1]-1] == 31):
            if (fecha[2] <= 30):
...
# Definición: Dada una fecha válida (año,mes,día),
# determinar cuál es la posición de esa fecha dentro del año.
# Por ejemplo: ordinal_dia(2021,1,1)=1, ordinal_dia(2020,3,1)=61,
# ordinal_dia(2020,2,29)
# Note que corresponde a 1 + el número de días transcurridos
# desde el primero de enero de ese año,
...
# que corresponde con la siguiente codificación:
...
# Dependiendo del siglo de un año dado, se obtiene
# otro número utilizado en la fórmula,
...
# Definición: Dado un año perteneciente al rango permitido,
# desplegar en consola el calendario de ese año
# en formato 3 secuencias ("filas") de 4 meses cada una.

# Funcion principal, se encarga de separar el problema en 4 meses
# cada iteración para poder imprimirlos en la función auxiliar
# También revisa si el año introducido es válido de acuerdo
# a las especificaciones del problema


def imprimir_3x4(anio):
    # Validación de año válido
    if type(anio) == int and anio >= institucion_calendario_gregoriano:
        if bisiesto(anio):
            print("Calendario del año {} D.C   =>  Año bisiesto   <=".format(anio))
            cantidades_meses[1] = 29
        else:
            print("Calendario del año {} D.C".format(anio))
        i = 0
        while i < 12:
            primer_dia = [0, 0, 0, 0]  # Arreglo del primer día de cada mes
            contador_mes = [1, 1, 1, 1]  # Arreglo contador de días de cada mes
            # Arreglo de banderas para lógica interna de cada mes
            banderas = [False, False, False, False]
            j = 0
            for j in range(4):
                # Asigna el primer día a cada mes
                primer_dia[j] = dia_semana((anio, (j+i)+1, 1))
            # Llama a función que se encarga de imprimir los 4 meses en línea
            imprimir_4_Meses(i, primer_dia, contador_mes, banderas)
            i += 4
        print(("-"*144))
        cantidades_meses[1] = 28

    else:
        print("ERROR => El valor del año introducido (", anio,
              ") debe ser de tipo entero positivo y ser mayor o igual al año ",
              institucion_calendario_gregoriano)

# Funcion auxiliar que se encarga de imprimir 4 meses
# dados en línea en la pantalla
# Recibe los arreglos provenientes de la función principal para trabajarlos
# y los modifica para la impresión de los valores


def imprimir_4_Meses(mes, primer_dia, contador_mes, banderas):
    indice_mes = dia = linea = 0
    print(("-"*144))
    # Imprime los nombres de los 4 meses
    print("{:^35}|{:^35}|{:^35}|{:^35}|".format(
        nombres_meses[mes], nombres_meses[mes+1],
        nombres_meses[mes+2], nombres_meses[mes+3]))
    i = 0
    for i in range(4):
        # Imprime las siglas de los 7 días de la semana para cada mes
        print("{:^5}{:^5}{:^5}{:^5}{:^5}{:^5}{:^5}|".format(
            'D', 'L', 'K', 'M', 'J', 'V', 'S'), end="")
    print("")
    # Recorre las 6 líneas de impresión de cada mes en forma vertical
    for linea in range(6):
        # Recorre los 4 meses para imprimir sus días en forma horizontal
        for indice_mes in range(4):
            # Recorre cada uno de los 7 días de la semana para
            # imprimir el valor
            for dia in range(7):
                if dia+1 <= primer_dia[indice_mes] and not banderas[indice_mes]:
                    # Si no se ha llegado el primer día del mes
                    # se imprime un espacio en blanco
                    print("{:^5}".format(" "), end="")
                elif contador_mes[indice_mes] > cantidades_meses[indice_mes+mes]:
                    # Si ya imprimió todos los días y quedan campos
                    # se imprime un espacio en blanco
                    print("{:^5}".format(""), end="")
                # Imprime el valor del día según las iteraciones
                else:
...
# Funcion auxiliar que se encarga de retornar un número si toca imprimir
# o en caso contrario un espacio en blanco para imprimir
# Recibe los arreglos provenientes de la la función auxiliar (imprimir_4_meses)
#  y retorna el número o un espacio en blanco según corresponda
...
# Definición: Dados una fecha válida f y un número entero no-negativo n,
...
def fecha_futura(fecha, dias):
    ...
    if type(dias) == int and dias >= 0 and fecha_es_valida(fecha):
        ...
        while dias != 0:
            ...
            # Si excede en la cantidad de días del mes actual
            if dia_temp > cantidades_meses[fecha[1] - 1]:
                # Días restantes a sumar
                dias = dias - (cantidades_meses[fecha[1] - 1] - dia_actual) - 1
                # En caso de que sea diciembre, se requiere cambiar de año
                if fecha[1] == 12:
                    fecha[0] += 1
                    fecha[1] = 1
                    fecha[2] = 1
                    # En caso de ser bisiesto, modificar febrero
                    if bisiesto(fecha[0]):
                        cantidades_meses[1] = 29
                    else:
                        cantidades_meses[1] = 28
...
# Definición: Dadas 2 fechas válidas, f1 y f2,
# sin importar si f1 <= f2 o f2 <= f1, determinar el número de
...
def dias_entre(fecha1, fecha2):
    # Se revisa si son fechas validas
    if fecha_es_valida(fecha1) and fecha_es_valida(fecha2):
        ...
        # Se usa la diferencia entre los años para determinar
        # cuantas veces se tiene que hacer el aumento
        # El año más pequeño es utilizado para saber si es bisiesto
        # Se hace un loop para aumentar la diferencia que hay entre los años
        for var in range(0, rep):
            # Se revisa si es bisiesto para aumentar el día extra
            if bisiesto(smallestYear[0] + var):
                diasExtra += 366
            else:
                diasExtra += 365

        # Se compara con la fecha mayor haciendo uso de la funcion auxiliar
        if fecha_mayor(fecha1, fecha2) == fecha1:
...
def fecha_futura_habil(fecha, dias):
    # Se hara un ciclo para ir agregando dias a las fechas validas
    for dia in range(0, dias):
        # Se hace esto en cada ciclo para ir verificando la valides de la fecha
        nFecha = dia_semana(fecha_futura(fecha, 1))
        # Se revisa si la fecha más 1 día es valida
        if nFecha != 6 and nFecha != 0:
            # Si es así se le suma el día
            fecha = fecha_futura(fecha, 1)
        else:
            # Se verifica si cae Sabado o Domingo
            if nFecha == 6:
                # Si la fecha cae Sabado con ese día de más,
                # se le suman 3 días para quedar en Lunes
                fecha = fecha_futura(fecha, 3)
            else:
                # Si la fecha cae Domingo con ese día de más,
                # se le suman 2 días para quedar en Lunes
                fecha = fecha_futura(fecha, 2)
    return fecha
# |------------------------------------------------------------------------|
# |                              Pruebas                                   |
# |------------------------------------------------------------------------|
...
def pruebaR7():
    ...
    dias3 = 366
    dias4 = 5840
    print("Fecha inicial: ", fecha1, "=>  Resultado de la fecha después de ",
          dias1, " dias: ", fecha_futura(fecha1, dias1))
    print("Fecha inicial: ", fecha2, "=>  Resultado de la fecha después de ",
          dias2, " dias: ", fecha_futura(fecha2, dias2))
    print("Fecha inicial: ", fecha3, "=>  Resultado de la fecha después de ",
          dias3, " dias: ", fecha_futura(fecha3, dias3))
    print("Fecha inicial: ", fecha4, "=>  Resultado de la fecha después de ",
          dias4, " dias: ", fecha_futura(fecha4, dias4))
...
def pruebaR8():
    ...
    fecha5 = (2015, 3, 1)
    fecha6 = (2016, 3, 1)
    print("Fecha1: ", fecha1, "Fecha 2: ", fecha1,
          "=> Resultado: ", dias_entre(fecha1, fecha1))
    print("Fecha1: ", fecha1, "Fecha 2: ", fecha2,
          "=> Resultado: ", dias_entre(fecha1, fecha2))
    print("Fecha1: ", fecha3, "Fecha 2: ", fecha4,
          "=> Resultado: ", dias_entre(fecha3, fecha4))
    print("Fecha1: ", fecha5, "Fecha 2: ", fecha6,
          "=> Resultado: ", dias_entre(fecha5, fecha6))
...
def pruebaR9():
    ...
    print("Fecha inicial: ", fecha1, "=>  Resultado de la fecha después de ",
          dias1, " dias hábiles: ", fecha_futura_habil(fecha1, dias1))
    print("Fecha inicial: ", fecha2, "=>  Resultado de la fecha después de ",
          dias2, " dias hábiles: ", fecha_futura_habil(fecha2, dias2))
    print("Fecha inicial: ", fecha3, "=>  Resultado de la fecha después de ",
          dias3, " dias hábiles: ", fecha_futura_habil(fecha3, dias3))
    print("Fecha inicial: ", fecha4, "=>  Resultado de la fecha después de ",
          dias4, " dias hábiles: ", fecha_futura_habil(fecha4, dias4))
