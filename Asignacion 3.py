...
Instituto Tecnológico de Costa Rica
    ...
    Erick Alfonso Elizondo Ramírez 
...
meses_31_dias=[1,3,5,7,8,10,12]         # Arreglo de meses que tienen 31 días completos
meses_30_dias=[4,6,9,11]                # Arreglo de meses que tienen 30 días completos
nombres_meses=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Setiembre","Octubre","Noviembre","Diciembre"]    #NUEVO (Steven) Arreglo de los nombres de los 12 meses del año
...
def fecha_es_tupla(fecha) :
    if len(fecha) == 3 :    #Se revisa si es una terna
        if isinstance(fecha[0], int) and isinstance(fecha[1], int) and isinstance(fecha[2], int):   #Se revisa si es una terna de enteros
...
def fecha_es_valida(fecha):
    if (len(fecha)==3):
        for date in fecha:
            if not isinstance(date, int) or date<0: return False
...
def validar_dia_mes(ano, mes , dia):
    if (mes in meses_31_dias and dia>=1): return (dia<=31) 
    if (mes in meses_30_dias and dia>=1): return (dia<=30) 
    if (mes==2 and dia>=1):
...
def dia_siguiente(fecha):
    if(fecha_es_valida(fecha)):   #Se valida si la fecha ingresada es válida, de lo contrario se retorna False
        if (fecha[1] in meses_31_dias): #Comprobación en los respectivos arreglos de meses
            if (fecha[2] <=30): return (fecha[0],fecha[1],fecha[2]+1) 
            return aumentar_mes(fecha)
        if (fecha[1] in meses_30_dias):
            if (fecha[2] <=29): return (fecha[0],fecha[1],fecha[2]+1) 
            return aumentar_mes(fecha)  #Se aumenta sólo el mes de manera normal 
        return aumentar_bisiesto(fecha)  #Se aumenta el día o mes considerando bisiestos
...
def ordinal_dia(fecha): 
    if fecha_es_valida(fecha):
        meses = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]  # Lista generada a partir de días que hay al mes
        cantidad_dias = 0
...
def imprimir_3x4(anio):
    if isinstance(anio, int) and anio >= institucion_calendario_gregoriano:  #Validación de año válido
        if bisiesto(anio): print("Calendario del año {} D.C   =>  Año bisiesto   <=".format(anio))
        ...
        while i<12:
            ...
            cantidades_meses=[0,0,0,0]                   #Arreglo cantidad de días de cada mes
            ...
            for j in range(4):
                ...
                cantidades_meses[j]=cantidad_dias(j+i,anio)         #Asigna la cantidad de días respectiva a cada mes
            imprimir_4_Meses(i,primer_dia,contador_mes,cantidades_meses,banderas)  #Llama a función que se encarga de imprimir los 4 meses en línea
            i+=4
        print(("-"*144))
...
def imprimir_4_Meses(mes,primer_dia,contador_mes,cantidades_meses,banderas):
    indice_mes=dia=0
    print(("-"*144))
    ...
    i = 0
    while( i < 4):
        print("{:^5}{:^5}{:^5}{:^5}{:^5}{:^5}{:^5}|".format('D','L','K','M','J','V','S'),end="")                      #Imprime las siglas de los 7 días de la semana para cada mes
        i += 1
    print("")

    linea = 0              
    while(linea < 6):                   #Recorre las 6 líneas de impresión de cada mes en forma vertical
        for indice_mes in range(4):     #Recorre los 4 meses para imprimir sus días en forma horizontal
            for dia in range(7):        #Recorre cada uno de los 7 días de la semana para imprimir el valor 
                if dia+1<=primer_dia[indice_mes] and banderas[indice_mes] is False:print("{:^5}".format(" "),end="")    #Si no se ha llegado el primer día del mes se imprime un espacio en blanco
                if contador_mes[indice_mes]>cantidades_meses[indice_mes]: print("{:^5}".format(""),end="")         #Si ya imprimió todos los días y quedan campos se imprime un espacio en blanco
...
#Funcion auxiliar para retornar la cantidad de días que tiene cada mes y evaluando también si el año es bisiesto o no
#Recibe un mes y el año y retorna la cantidad de días que tiene este mes para usarlo como contador en la impresión del calendario
def cantidad_dias(mes,anio):
    if mes+1==2:
        if bisiesto(anio):return 29
        return 28
    if mes+1 in meses_30_dias:return 30
    return 31
...
def fecha_futura(fecha,dias):
    if isinstance(dias, int) and dias>=0 and fecha_es_valida(fecha): #Verificación de datos
        ...
        while dias != 0:
            ...
            if dia_temp > meses[fecha[1] - 1]: #Si excede en la cantidad de días del mes actual
                dias = dias - (meses[fecha[1] - 1] - dia_actual) - 1 #Días restantes a sumar
...
def fecha_mayor(fecha1, fecha2):
    if fecha1[0] > fecha2[0]:           #Se comparan los años
        return fecha1
    if fecha1[0] == fecha2[0]:        #Se verifica si son años iguales
        if fecha1[1] > fecha2[1]:       #Se comparan los meses
            return fecha1
        if fecha1[1] == fecha2[1]:    #Se verifica si son meses iguales
...
def fecha_futura_habil(fecha,dias):
    cont = 0
    while(cont < dias):                             # Se hara un ciclo para ir agregando dias a las fechas validas
        nFecha = dia_semana(fecha_futura(fecha, 1)) # Se hace esto en cada ciclo para ir verificando la valides de la fecha
        if nFecha not in(0,6):                      # Se revisa si la fecha más 1 día es valida
